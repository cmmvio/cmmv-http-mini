...
export class HttpMini extends EventEmitter {
    ...
    public async handler(req: ServerRequest, res: ServerResponse) {
        ...
        if (route && route.handler) {
            try {
                //@ts-ignore
                const request = Request.Pack(req, route.params, route.searchParams, this);
                const response = Response.Pack(res, request, this);
    ...
    public printLog(
        ...
        if(this.option("debug")){
            ...
            switch (type) {
                case 'error': this.logger.error(logContent); break;
                case 'warning': this.logger.warning(logContent); break;
                case 'verbose': this.logger.verbose(logContent); break;
                default: this.logger.log(logContent); break;
            }
        }
    }
}
